...
def window_partition(x, window_size):
    ...
    """
    b, h, w, c = x.shape     #4, 336, 336, 2304
    x = x.view(b, h // window_size, window_size, w // window_size, window_size, c)    # 4, 24, 14, 24, 14, 1304
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size, c)  # 4*24*24, 14, 14, 1304
    return windows
...
class ASSM(nn.Module):
    ...
    def forward(self, x, x_size, token):
        ...
        cls_policy = F.gumbel_softmax(pred_route, hard=True, dim=-1)  # [B, HW, num_token] num_token=64
...
class AttentiveLayer(nn.Module):
    ...
    def forward(self, x, x_size, params):
        ...
        x = self.norm1(x)
        qkv = self.wqkv(x)
        qkv = qkv.reshape(b, h, w, c3)  #4,336,336,2304
        if self.shift_size > 0:
            ...
            shifted_qkv = qkv
            attn_mask = None
        x_windows = window_partition(shifted_qkv, self.window_size)                     ## out shape 4*24*24, 14, 14, 2304
        x_windows = x_windows.view(-1, self.window_size * self.window_size, c3)         ## 2304,196,2304
        attn_windows = self.win_mhsa(x_windows, rpi=params['rpi_sa'], mask=attn_mask)   # 2304, 196,768
        attn_windows = attn_windows.view(-1, self.window_size, self.window_size, c)     # 2304,14,14,768
        shifted_x = window_reverse(attn_windows, self.window_size, h, w)  # b h' w' c   4,336,336,768
        if self.shift_size > 0:
            attn_x = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            attn_x = shifted_x
        x_win = attn_x.view(b,n,c) + shortcut                          # 4, 112896, 768
        x_win = self.convffn1(self.norm2(x_win), x_size) + x_win       # 4, 112896, 768
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, x_size, params):
        b, n, c = x.shape          #b:4, n:112896, c:768
...
class MambaIRv2(nn.Module):
    ...
    def forward_features(self, x, params):
        x_size = (x.shape[2], x.shape[3]) # 336,336
        x = self.patch_embed(x)  # b, seq_len, c 4,336*336, 768
        if self.ape:
            x = x + self.absolute_pos_embed

        for layer in self.layers:                         ########TODO first x shape b, h*w, c
            x = layer(x, x_size, params)
    ...
    def forward(self, x):
        # padding
        h_ori, w_ori = x.size()[-2], x.size()[-1]               #336, 336
        mod = self.window_size                                  #14
        h_pad = ((h_ori + mod - 1) // mod) * mod - h_ori        #0 
        w_pad = ((w_ori + mod - 1) // mod) * mod - w_ori        #0
        ...
        attn_mask = self.calculate_mask([h, w]).to(x.device)  #h:336,w:336
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for lightweight SR
            x = self.conv_first(x)  #input shape:x b,c,h,w   out shape: b,embed,h,w    4,768,336,336
            x = self.conv_after_body(self.forward_features(x, params)) + x      #TODO 
            x = self.upsample(x)
        elif self.upsampler == 'nearest+conv':
            # for real-world SR
            x = self.conv_first(x)
...
def buildMambaIRv2_tiny(upscale=2, embed_dim=48):
    return MambaIRv2(img_size=64,
                   patch_size=1,
                   in_chans=3,
                   embed_dim=embed_dim,
