...
class AttentiveLayer(nn.Module):
    ...
    def forward(self, x, x_size, params):
        # h, w = x_size
        # h, w = h // self.window_size, w // self.window_size   #ADD 
...
class MambaIRv2(nn.Module):
    ...
    def forward_features(self, x, params=None, x_size=None):
        # x_size = (x.shape[2], x.shape[3])
        # x = self.patch_embed(x)
        if x_size is None:
            w = int(np.sqrt(x.shape[-2]))
            x_size = (int(w), int(w))
    ...
    def forward(self, x, x_size=None):
        ...
        # assert h_pad >= 0 and w_pad >= 0 and h_pad % mod == 0 and w_pad % mod == 0, f"h_pad {h_pad}, w_pad {w_pad} must be a multiple of window_size {mod}"
        ...
        # x = self.upsample(x)
        # shut_cut = x
        out = self.forward_features(x, x_size=x_size)
        # hidden_states = self.conv_after_body(out) + out
        hidden_states = self.conv_after_body(out)

        # out = out+ x                                                    #TODO 
        # x = self.conv_after_body(self.forward_features(x, x_size=x_size)) + x      #TODO 
        # x = self.upsample(hidden_states)
        ...
        # return x, torch.flatten(hidden_states, start_dim=2).transpose(1, 2)
        return torch.flatten(hidden_states, start_dim=2).transpose(1, 2)
